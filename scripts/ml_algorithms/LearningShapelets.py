...
# Aplicar a média móvel para todas as linhas
X_smooth = np.zeros_like(X)  # Inicializar uma matriz para armazenar os resultados suavizados

for i in range(X.shape[0]):
    serie = pd.Series(X[i])
    rolling = serie.rolling(window=window_size)
    serie_smooth = rolling.mean().to_numpy()
    X_smooth[i] = serie_smooth # Agora X_smooth contém as séries temporais suavizadas

# Normalize each of the timeseries
X_smooth = TimeSeriesScalerMinMax().fit_transform(X)

# Get statistics of the dataset
n_ts, ts_sz = X_smooth.shape[:2]
n_classes = len(set(y))
...
shp_clf = LearningShapelets(n_shapelets_per_size=shapelet_sizes,
                            ...
                            random_state=1,
                            verbose=0)
shp_clf.fit(X_smooth, y)

# Make predictions and calculate accuracy score
pred_labels = shp_clf.predict(X_smooth)
